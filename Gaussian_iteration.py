#Gaussian Method
# In this method the given matrix can be written A=D(diagonal)-L(strictly lower traingular)-U(strictly upper triangular) and the soulution of the equation A*x=b is  X(k+1)=U*(D-L)^(-1)*X(k)+(D-L)^(-1)*b   where k is the iteration 
import numpy as np
...
D_=np.linalg.inv(D-L) #D_ is the inverse of (D-L)^(-1) via numpy.linalg.inv
count=0#iteration
key=0
for h in range(200):
    if(key==1):   #condition of the process to next iteration
        X=np.matmul(np.matmul(D_,U),X)+np.matmul(D_,b)#solution after next iteration 
        count=count+1
    key=0
    for k in range(5):            
...
if(key==1):
    print('The required accuracy is not reached yet after ',count,' iterations')
else:
