#Relaxation Method 
# In this method the given matrix can be written A=D(diagonal)-L(strictly lower traingular)-U(strictly upper triangular) and the soulution of the equation A*x=b is  X(k+1)=(D-w*L)^(-1)[(1-w)*D+w*U]*X(k)+w*(D-w*L)^(-1)*b   where k is the iteration
import numpy as np
...
D_=np.linalg.inv(D-w*L)#D_ is the inverse of (D-w*L)^(-1) via numpy.linalg.inv
count=0#iteration
key=0

for h in range(100):
    if(key==1):  #condition of the process to next iteration
        F=np.matmul(D_,(1-w)*D+w*U)
        C=w*D_  
        X=np.matmul(F,X)+np.matmul(C,b)#solution after next iteration 
        count=count+1
    key=0
    for k in range(5):            
